...
const records = [
  { action: '完成单词练习', xp: '+10 XP', time: '10分钟前', icon: BookOpen, color: 'text-sky-500', to: '/learn/word-cards' },
  { action: '听说训练评分A', xp: '+20 XP', time: '1小时前', icon: Mic, color: 'text-green-500', to: '/learn/listening-speaking' },
  { action: '完成每日英语', xp: '+15 XP', time: '2小时前', icon: Target, color: 'text-teal-500', to: '/learn/english-daily' },
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-sm border border-white/60 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-2xl font-bold text-green-600">1,280</span>
          </div>
          <div className="w-full bg-green-100 rounded-full h-2.5">
            <div className="bg-gradient-to-r from-green-400 to-emerald-500 h-2.5 rounded-full" style={{ width: '42%' }} />
          </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-sm border border-white/60 p-6">
          ...
          <div className="space-y-4">
            {records.map((r, i) => (
              <Link key={i} to={r.to} className="flex items-center justify-between py-2 border-b border-green-50 last:border-0 hover:bg-green-50/50 px-2 -mx-2 rounded-lg transition-colors">
                <div className="flex items-center gap-3">
                  <r.icon className={`w-5 h-5 ${r.color}`} />
                  <span className="text-sm text-slate-700">{r.action}</span>
                </div>
                <div className="text-right">
                  <span className="text-sm font-semibold text-green-600">{r.xp}</span>
